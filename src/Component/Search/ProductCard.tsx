/*
 * @Author: 不见霞 15550238+yvi-ksm@user.noreply.gitee.com
 * @Date: 2025-11-27 15:58:55
 * @LastEditors: 不见霞 15550238+yvi-ksm@user.noreply.gitee.com
 * @LastEditTime: 2025-11-27 16:29:48
 * @FilePath: \lenovo-shop\src\component\Search\ProductCard.tsx
 * @Description: 
 * 
 * Copyright (c) 2025 by ${git_name_email}, All Rights Reserved. 
 */
// components/Search/ProductCard.tsx
import React from 'react';
import { Link } from 'react-router-dom';
import Tag from '../Search/Tag';
import type { Product } from '../../types/searchProduct';



interface ProductCardProps {
    product: Product;
}

const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
    return (
        <li className='w-[290px] h-[385px] bg-white hover:shadow-lg transition-all duration-300'>
            <div className='px-4 py-4'>
                <Link to={product.link} className='cursor-pointer'>
                    {/* 商品图片 */}
                    <div className='overflow-hidden w-[250px] h-[180px] mx-auto border-b-[1px] border-[#e0e0e0] flex items-center justify-center'>
                        <img
                            className='w-[160px] h-[160px] object-contain'
                            src={product.image}
                            alt={product.name}
                        />
                    </div>

                    {/* 商品名称 */}
                    <div className='leading-4 px-1 h-[36px] mt-[10px] text-left text-[#424242]'>
                        <span className='text-[13px] line-clamp-2'>
                            {product.name}
                        </span>
                    </div>

                    {/* 商品描述 */}
                    <div className='h-9 px-1 leading-4 overflow-hidden mt-1.5 text-left text-[#cfcfcf] text-[12px] line-clamp-2'>
                        {product.description}
                    </div>

                    {/* 价格信息 */}
                    <div className='flex items-center mt-1 px-1'>
                        <div className='font-bold h-[30px] flex text-[14px] text-red-500 items-center'>
                            <span>{product.isDiscount && "到手价"}￥{product.currentPrice}</span>
                        </div>
                        {product.isDiscount && <div className='text-[12px] text-[#979797] mt-0.5 ml-[3px] line-through'>
                            <span>￥{product.originalPrice}</span>
                        </div>}
                    </div>

                    {/* 标签区域 */}
                    <div className='min-h-[30px] leading-[30px] overflow-hidden flex items-center flex-wrap'>
                        {product.tags.self && <Tag type="self" />}
                        {product.tags.coupon && <Tag type="coupon" money={product.tags.coupon.money} />}
                        {product.tags.custom && <Tag type="custom" />}
                        {product.tags.tradeIn && <Tag type="tradeIn" />}
                        {product.tags.installment && <Tag type="installment" month={product.tags.installment.month} />}
                    </div>
                </Link>
            </div>
        </li>
    );
};

export default ProductCard;
