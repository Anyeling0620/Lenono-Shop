    ...
    return (
        <li className='w-[290px] h-[385px] bg-white hover:shadow-lg transition-all duration-300'>
            <div className='px-4 py-4'>
                <Link to={`/product/${product.shelfProduct.id}`} className='cursor-pointer' target={product.shelfProduct.id}>
                        ...
                        <div className='flex items-center mt-1 px-1'>
                            <div className='font-bold h-[30px] flex text-[14px] text-red-500 items-center'>
                                <span>{product.coupons && "到手价"}￥{product.minPriceConfig.salePrice}</span>
                            </div>
                            {product.coupons && <div className='text-[12px] text-[#979797] mt-0.5 ml-[3px] line-through'>
                        ...
                        <div className='min-h-[30px] leading-[30px] overflow-hidden flex items-center flex-wrap'>
                            ...
                            {/* {product.coupons && <Tag type="coupon" money={product.coupons[0]?.coupon.type === '满减'? product.coupons[0].coupon.amount! : (1-(product.coupons[0].coupon.amount as number))*(product.minPriceConfig.salePrice as number) as number} />} */}
